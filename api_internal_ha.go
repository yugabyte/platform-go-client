/*
YugabyteDB Anywhere APIs

ALPHA - NOT FOR EXTERNAL USE

API version: v1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package ywclient

import (
	"bytes"
	"context"
	"io"
	"net/http"
	"net/url"
	"strings"
)

// InternalHAAPIService InternalHAAPI service
type InternalHAAPIService service

type InternalHAAPIDemoteLocalLeaderRequest struct {
	ctx        context.Context
	ApiService *InternalHAAPIService
	timestamp  int64
	promote    *bool
	request    *interface{}
}

func (r InternalHAAPIDemoteLocalLeaderRequest) Promote(promote bool) InternalHAAPIDemoteLocalLeaderRequest {
	r.promote = &promote
	return r
}

func (r InternalHAAPIDemoteLocalLeaderRequest) Request(request interface{}) InternalHAAPIDemoteLocalLeaderRequest {
	r.request = &request
	return r
}

func (r InternalHAAPIDemoteLocalLeaderRequest) Execute() (*PlatformInstance, *http.Response, error) {
	return r.ApiService.DemoteLocalLeaderExecute(r)
}

/*
DemoteLocalLeader Demote local leader

Available since YBA version 2.20.0.

	@param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
	@param timestamp
	@return InternalHAAPIDemoteLocalLeaderRequest
*/
func (a *InternalHAAPIService) DemoteLocalLeader(ctx context.Context, timestamp int64) InternalHAAPIDemoteLocalLeaderRequest {
	return InternalHAAPIDemoteLocalLeaderRequest{
		ApiService: a,
		ctx:        ctx,
		timestamp:  timestamp,
	}
}

// Execute executes the request
//
//	@return PlatformInstance
func (a *InternalHAAPIService) DemoteLocalLeaderExecute(r InternalHAAPIDemoteLocalLeaderRequest) (*PlatformInstance, *http.Response, error) {
	var (
		localVarHTTPMethod  = http.MethodPut
		localVarPostBody    interface{}
		formFiles           []formFile
		localVarReturnValue *PlatformInstance
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "InternalHAAPIService.DemoteLocalLeader")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/api/v1/settings/ha/internal/config/demote/{timestamp}"
	localVarPath = strings.Replace(localVarPath, "{"+"timestamp"+"}", url.PathEscape(parameterValueToString(r.timestamp, "timestamp")), -1)

	localVarHeaderParams := make(map[string]string)
	localVarQueryParams := url.Values{}
	localVarFormParams := url.Values{}

	if r.promote != nil {
		parameterAddToHeaderOrQuery(localVarQueryParams, "promote", r.promote, "", "")
	} else {
		var defaultValue bool = false
		parameterAddToHeaderOrQuery(localVarQueryParams, "promote", defaultValue, "", "")
		r.promote = &defaultValue
	}
	if r.request != nil {
		parameterAddToHeaderOrQuery(localVarQueryParams, "request", r.request, "", "")
	}
	// to determine the Content-Type header
	localVarHTTPContentTypes := []string{}

	// set Content-Type header
	localVarHTTPContentType := selectHeaderContentType(localVarHTTPContentTypes)
	if localVarHTTPContentType != "" {
		localVarHeaderParams["Content-Type"] = localVarHTTPContentType
	}

	// to determine the Accept header
	localVarHTTPHeaderAccepts := []string{"application/json"}

	// set Accept header
	localVarHTTPHeaderAccept := selectHeaderAccept(localVarHTTPHeaderAccepts)
	if localVarHTTPHeaderAccept != "" {
		localVarHeaderParams["Accept"] = localVarHTTPHeaderAccept
	}
	req, err := a.client.prepareRequest(r.ctx, localVarPath, localVarHTTPMethod, localVarPostBody, localVarHeaderParams, localVarQueryParams, localVarFormParams, formFiles)
	if err != nil {
		return localVarReturnValue, nil, err
	}

	localVarHTTPResponse, err := a.client.callAPI(req)
	if err != nil || localVarHTTPResponse == nil {
		return localVarReturnValue, localVarHTTPResponse, err
	}

	localVarBody, err := io.ReadAll(localVarHTTPResponse.Body)
	localVarHTTPResponse.Body.Close()
	localVarHTTPResponse.Body = io.NopCloser(bytes.NewBuffer(localVarBody))
	if err != nil {
		return localVarReturnValue, localVarHTTPResponse, err
	}

	if localVarHTTPResponse.StatusCode >= 300 {
		newErr := &GenericOpenAPIError{
			body:  localVarBody,
			error: localVarHTTPResponse.Status,
		}
		return localVarReturnValue, localVarHTTPResponse, newErr
	}

	err = a.client.decode(&localVarReturnValue, localVarBody, localVarHTTPResponse.Header.Get("Content-Type"))
	if err != nil {
		newErr := &GenericOpenAPIError{
			body:  localVarBody,
			error: err.Error(),
		}
		return localVarReturnValue, localVarHTTPResponse, newErr
	}

	return localVarReturnValue, localVarHTTPResponse, nil
}

type InternalHAAPIGetHAConfigByClusterKeyRequest struct {
	ctx        context.Context
	ApiService *InternalHAAPIService
	request    *interface{}
}

func (r InternalHAAPIGetHAConfigByClusterKeyRequest) Request(request interface{}) InternalHAAPIGetHAConfigByClusterKeyRequest {
	r.request = &request
	return r
}

func (r InternalHAAPIGetHAConfigByClusterKeyRequest) Execute() (*http.Response, error) {
	return r.ApiService.GetHAConfigByClusterKeyExecute(r)
}

/*
GetHAConfigByClusterKey Method for GetHAConfigByClusterKey

	@param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
	@return InternalHAAPIGetHAConfigByClusterKeyRequest
*/
func (a *InternalHAAPIService) GetHAConfigByClusterKey(ctx context.Context) InternalHAAPIGetHAConfigByClusterKeyRequest {
	return InternalHAAPIGetHAConfigByClusterKeyRequest{
		ApiService: a,
		ctx:        ctx,
	}
}

// Execute executes the request
func (a *InternalHAAPIService) GetHAConfigByClusterKeyExecute(r InternalHAAPIGetHAConfigByClusterKeyRequest) (*http.Response, error) {
	var (
		localVarHTTPMethod = http.MethodGet
		localVarPostBody   interface{}
		formFiles          []formFile
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "InternalHAAPIService.GetHAConfigByClusterKey")
	if err != nil {
		return nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/api/v1/settings/ha/internal/config"

	localVarHeaderParams := make(map[string]string)
	localVarQueryParams := url.Values{}
	localVarFormParams := url.Values{}

	if r.request != nil {
		parameterAddToHeaderOrQuery(localVarQueryParams, "request", r.request, "", "")
	}
	// to determine the Content-Type header
	localVarHTTPContentTypes := []string{}

	// set Content-Type header
	localVarHTTPContentType := selectHeaderContentType(localVarHTTPContentTypes)
	if localVarHTTPContentType != "" {
		localVarHeaderParams["Content-Type"] = localVarHTTPContentType
	}

	// to determine the Accept header
	localVarHTTPHeaderAccepts := []string{}

	// set Accept header
	localVarHTTPHeaderAccept := selectHeaderAccept(localVarHTTPHeaderAccepts)
	if localVarHTTPHeaderAccept != "" {
		localVarHeaderParams["Accept"] = localVarHTTPHeaderAccept
	}
	req, err := a.client.prepareRequest(r.ctx, localVarPath, localVarHTTPMethod, localVarPostBody, localVarHeaderParams, localVarQueryParams, localVarFormParams, formFiles)
	if err != nil {
		return nil, err
	}

	localVarHTTPResponse, err := a.client.callAPI(req)
	if err != nil || localVarHTTPResponse == nil {
		return localVarHTTPResponse, err
	}

	localVarBody, err := io.ReadAll(localVarHTTPResponse.Body)
	localVarHTTPResponse.Body.Close()
	localVarHTTPResponse.Body = io.NopCloser(bytes.NewBuffer(localVarBody))
	if err != nil {
		return localVarHTTPResponse, err
	}

	if localVarHTTPResponse.StatusCode >= 300 {
		newErr := &GenericOpenAPIError{
			body:  localVarBody,
			error: localVarHTTPResponse.Status,
		}
		return localVarHTTPResponse, newErr
	}

	return localVarHTTPResponse, nil
}

type InternalHAAPISyncBackupsRequest struct {
	ctx        context.Context
	ApiService *InternalHAAPIService
	request    *interface{}
}

func (r InternalHAAPISyncBackupsRequest) Request(request interface{}) InternalHAAPISyncBackupsRequest {
	r.request = &request
	return r
}

func (r InternalHAAPISyncBackupsRequest) Execute() (*http.Response, error) {
	return r.ApiService.SyncBackupsExecute(r)
}

/*
SyncBackups Method for SyncBackups

	@param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
	@return InternalHAAPISyncBackupsRequest
*/
func (a *InternalHAAPIService) SyncBackups(ctx context.Context) InternalHAAPISyncBackupsRequest {
	return InternalHAAPISyncBackupsRequest{
		ApiService: a,
		ctx:        ctx,
	}
}

// Execute executes the request
func (a *InternalHAAPIService) SyncBackupsExecute(r InternalHAAPISyncBackupsRequest) (*http.Response, error) {
	var (
		localVarHTTPMethod = http.MethodPost
		localVarPostBody   interface{}
		formFiles          []formFile
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "InternalHAAPIService.SyncBackups")
	if err != nil {
		return nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/api/v1/settings/ha/internal/upload"

	localVarHeaderParams := make(map[string]string)
	localVarQueryParams := url.Values{}
	localVarFormParams := url.Values{}

	if r.request != nil {
		parameterAddToHeaderOrQuery(localVarQueryParams, "request", r.request, "", "")
	}
	// to determine the Content-Type header
	localVarHTTPContentTypes := []string{}

	// set Content-Type header
	localVarHTTPContentType := selectHeaderContentType(localVarHTTPContentTypes)
	if localVarHTTPContentType != "" {
		localVarHeaderParams["Content-Type"] = localVarHTTPContentType
	}

	// to determine the Accept header
	localVarHTTPHeaderAccepts := []string{}

	// set Accept header
	localVarHTTPHeaderAccept := selectHeaderAccept(localVarHTTPHeaderAccepts)
	if localVarHTTPHeaderAccept != "" {
		localVarHeaderParams["Accept"] = localVarHTTPHeaderAccept
	}
	req, err := a.client.prepareRequest(r.ctx, localVarPath, localVarHTTPMethod, localVarPostBody, localVarHeaderParams, localVarQueryParams, localVarFormParams, formFiles)
	if err != nil {
		return nil, err
	}

	localVarHTTPResponse, err := a.client.callAPI(req)
	if err != nil || localVarHTTPResponse == nil {
		return localVarHTTPResponse, err
	}

	localVarBody, err := io.ReadAll(localVarHTTPResponse.Body)
	localVarHTTPResponse.Body.Close()
	localVarHTTPResponse.Body = io.NopCloser(bytes.NewBuffer(localVarBody))
	if err != nil {
		return localVarHTTPResponse, err
	}

	if localVarHTTPResponse.StatusCode >= 300 {
		newErr := &GenericOpenAPIError{
			body:  localVarBody,
			error: localVarHTTPResponse.Status,
		}
		return localVarHTTPResponse, newErr
	}

	return localVarHTTPResponse, nil
}

type InternalHAAPISyncInstancesRequest struct {
	ctx        context.Context
	ApiService *InternalHAAPIService
	timestamp  int64
	request    *interface{}
}

func (r InternalHAAPISyncInstancesRequest) Request(request interface{}) InternalHAAPISyncInstancesRequest {
	r.request = &request
	return r
}

func (r InternalHAAPISyncInstancesRequest) Execute() (*http.Response, error) {
	return r.ApiService.SyncInstancesExecute(r)
}

/*
SyncInstances Method for SyncInstances

	@param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
	@param timestamp
	@return InternalHAAPISyncInstancesRequest
*/
func (a *InternalHAAPIService) SyncInstances(ctx context.Context, timestamp int64) InternalHAAPISyncInstancesRequest {
	return InternalHAAPISyncInstancesRequest{
		ApiService: a,
		ctx:        ctx,
		timestamp:  timestamp,
	}
}

// Execute executes the request
func (a *InternalHAAPIService) SyncInstancesExecute(r InternalHAAPISyncInstancesRequest) (*http.Response, error) {
	var (
		localVarHTTPMethod = http.MethodPut
		localVarPostBody   interface{}
		formFiles          []formFile
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "InternalHAAPIService.SyncInstances")
	if err != nil {
		return nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/api/v1/settings/ha/internal/config/sync/{timestamp}"
	localVarPath = strings.Replace(localVarPath, "{"+"timestamp"+"}", url.PathEscape(parameterValueToString(r.timestamp, "timestamp")), -1)

	localVarHeaderParams := make(map[string]string)
	localVarQueryParams := url.Values{}
	localVarFormParams := url.Values{}

	if r.request != nil {
		parameterAddToHeaderOrQuery(localVarQueryParams, "request", r.request, "", "")
	}
	// to determine the Content-Type header
	localVarHTTPContentTypes := []string{}

	// set Content-Type header
	localVarHTTPContentType := selectHeaderContentType(localVarHTTPContentTypes)
	if localVarHTTPContentType != "" {
		localVarHeaderParams["Content-Type"] = localVarHTTPContentType
	}

	// to determine the Accept header
	localVarHTTPHeaderAccepts := []string{}

	// set Accept header
	localVarHTTPHeaderAccept := selectHeaderAccept(localVarHTTPHeaderAccepts)
	if localVarHTTPHeaderAccept != "" {
		localVarHeaderParams["Accept"] = localVarHTTPHeaderAccept
	}
	req, err := a.client.prepareRequest(r.ctx, localVarPath, localVarHTTPMethod, localVarPostBody, localVarHeaderParams, localVarQueryParams, localVarFormParams, formFiles)
	if err != nil {
		return nil, err
	}

	localVarHTTPResponse, err := a.client.callAPI(req)
	if err != nil || localVarHTTPResponse == nil {
		return localVarHTTPResponse, err
	}

	localVarBody, err := io.ReadAll(localVarHTTPResponse.Body)
	localVarHTTPResponse.Body.Close()
	localVarHTTPResponse.Body = io.NopCloser(bytes.NewBuffer(localVarBody))
	if err != nil {
		return localVarHTTPResponse, err
	}

	if localVarHTTPResponse.StatusCode >= 300 {
		newErr := &GenericOpenAPIError{
			body:  localVarBody,
			error: localVarHTTPResponse.Status,
		}
		return localVarHTTPResponse, newErr
	}

	return localVarHTTPResponse, nil
}

type InternalHAAPIValidateBackupRequest struct {
	ctx        context.Context
	ApiService *InternalHAAPIService
	backupName string
	request    *interface{}
}

func (r InternalHAAPIValidateBackupRequest) Request(request interface{}) InternalHAAPIValidateBackupRequest {
	r.request = &request
	return r
}

func (r InternalHAAPIValidateBackupRequest) Execute() (*http.Response, error) {
	return r.ApiService.ValidateBackupExecute(r)
}

/*
ValidateBackup Method for ValidateBackup

	@param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
	@param backupName
	@return InternalHAAPIValidateBackupRequest
*/
func (a *InternalHAAPIService) ValidateBackup(ctx context.Context, backupName string) InternalHAAPIValidateBackupRequest {
	return InternalHAAPIValidateBackupRequest{
		ApiService: a,
		ctx:        ctx,
		backupName: backupName,
	}
}

// Execute executes the request
func (a *InternalHAAPIService) ValidateBackupExecute(r InternalHAAPIValidateBackupRequest) (*http.Response, error) {
	var (
		localVarHTTPMethod = http.MethodGet
		localVarPostBody   interface{}
		formFiles          []formFile
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "InternalHAAPIService.ValidateBackup")
	if err != nil {
		return nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/api/v1/settings/ha/internal/backups/{backupName}/validate"
	localVarPath = strings.Replace(localVarPath, "{"+"backupName"+"}", url.PathEscape(parameterValueToString(r.backupName, "backupName")), -1)

	localVarHeaderParams := make(map[string]string)
	localVarQueryParams := url.Values{}
	localVarFormParams := url.Values{}

	if r.request != nil {
		parameterAddToHeaderOrQuery(localVarQueryParams, "request", r.request, "", "")
	}
	// to determine the Content-Type header
	localVarHTTPContentTypes := []string{}

	// set Content-Type header
	localVarHTTPContentType := selectHeaderContentType(localVarHTTPContentTypes)
	if localVarHTTPContentType != "" {
		localVarHeaderParams["Content-Type"] = localVarHTTPContentType
	}

	// to determine the Accept header
	localVarHTTPHeaderAccepts := []string{}

	// set Accept header
	localVarHTTPHeaderAccept := selectHeaderAccept(localVarHTTPHeaderAccepts)
	if localVarHTTPHeaderAccept != "" {
		localVarHeaderParams["Accept"] = localVarHTTPHeaderAccept
	}
	req, err := a.client.prepareRequest(r.ctx, localVarPath, localVarHTTPMethod, localVarPostBody, localVarHeaderParams, localVarQueryParams, localVarFormParams, formFiles)
	if err != nil {
		return nil, err
	}

	localVarHTTPResponse, err := a.client.callAPI(req)
	if err != nil || localVarHTTPResponse == nil {
		return localVarHTTPResponse, err
	}

	localVarBody, err := io.ReadAll(localVarHTTPResponse.Body)
	localVarHTTPResponse.Body.Close()
	localVarHTTPResponse.Body = io.NopCloser(bytes.NewBuffer(localVarBody))
	if err != nil {
		return localVarHTTPResponse, err
	}

	if localVarHTTPResponse.StatusCode >= 300 {
		newErr := &GenericOpenAPIError{
			body:  localVarBody,
			error: localVarHTTPResponse.Status,
		}
		return localVarHTTPResponse, newErr
	}

	return localVarHTTPResponse, nil
}
