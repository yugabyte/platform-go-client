...
type MetricSettings struct {
	...
	// Way of metrics aggregation across nodes
	NodeAggregation *string `json:"nodeAggregation,omitempty"`
	// Defines if we return 'mean' line with node lines in case we split nodes
	ReturnAggregatedValue *bool `json:"returnAggregatedValue,omitempty"`
	// Defines how many node lines we return in case we split by nodes/tables/etc.
	SplitCount *int32 `json:"splitCount,omitempty"`
	// Controls if we split nodes, tables, etc. into own lines OR aggregate  and how we select lines in case of split query
	SplitMode *string `json:"splitMode,omitempty"`
	// Defines set of labels, which we use for a split query
	SplitType *string `json:"splitType,omitempty"`
	// Way of metrics aggregation over time
