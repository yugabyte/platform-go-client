/*
YugabyteDB Anywhere APIs

ALPHA - NOT FOR EXTERNAL USE

API version: v1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package ywclient

import (
	"encoding/json"
)

// checks if the ProvisionUniverseNodesParams type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ProvisionUniverseNodesParams{}

// ProvisionUniverseNodesParams struct for ProvisionUniverseNodesParams
type ProvisionUniverseNodesParams struct {
	AdditionalServicesStateData *AdditionalServicesStateData `json:"additionalServicesStateData,omitempty"`
	AllowInsecure               *bool                        `json:"allowInsecure,omitempty"`
	Arch                        *string                      `json:"arch,omitempty"`
	AutoRollbackPerformed       *bool                        `json:"autoRollbackPerformed,omitempty"`
	Capability                  *string                      `json:"capability,omitempty"`
	ClientRootCA                *string                      `json:"clientRootCA,omitempty"`
	Clusters                    []Cluster                    `json:"clusters"`
	// Amazon Resource Name (ARN) of the CMK
	CmkArn                 *string                 `json:"cmkArn,omitempty"`
	CommunicationPorts     *CommunicationPorts     `json:"communicationPorts,omitempty"`
	CreatingUser           Users                   `json:"creatingUser"`
	CurrentClusterType     *string                 `json:"currentClusterType,omitempty"`
	DeviceInfo             *DeviceInfo             `json:"deviceInfo,omitempty"`
	EnableYbc              *bool                   `json:"enableYbc,omitempty"`
	EncryptionAtRestConfig *EncryptionAtRestConfig `json:"encryptionAtRestConfig,omitempty"`
	// Error message
	ErrorString *string `json:"errorString,omitempty"`
	// Expected universe version
	ExpectedUniverseVersion *int32             `json:"expectedUniverseVersion,omitempty"`
	ExtraDependencies       *ExtraDependencies `json:"extraDependencies,omitempty"`
	// YbaApi Internal. FIPS compatibility is enabled for universe
	FipsEnabled   *bool   `json:"fipsEnabled,omitempty"`
	ImportedState *string `json:"importedState,omitempty"`
	// YbaApi Internal. Install node agent in background if it is true
	InstallNodeAgent               *bool `json:"installNodeAgent,omitempty"`
	InstallYbc                     *bool `json:"installYbc,omitempty"`
	IsKubernetesOperatorControlled *bool `json:"isKubernetesOperatorControlled,omitempty"`
	// Available since YBA version 2.20.2.0
	IsSoftwareRollbackAllowed  *bool   `json:"isSoftwareRollbackAllowed,omitempty"`
	ItestS3PackagePath         *string `json:"itestS3PackagePath,omitempty"`
	KubernetesUpgradeSupported bool    `json:"kubernetesUpgradeSupported"`
	// <b style=\"color:#ff0000\">Deprecated since YBA version 2025.2.</b> With geo partitioning support, default region is replaced with default partition
	MastersInDefaultRegion *bool  `json:"mastersInDefaultRegion,omitempty"`
	NextClusterIndex       *int32 `json:"nextClusterIndex,omitempty"`
	// YbaApi Internal. True if a node agent for missing in any of the nodes
	NodeAgentMissing *bool `json:"nodeAgentMissing,omitempty"`
	// Node details
	NodeDetailsSet []NodeDetails `json:"nodeDetailsSet,omitempty"`
	// Node exporter user
	NodeExporterUser *string `json:"nodeExporterUser,omitempty"`
	// Node names to provision (empty means all nodes)
	NodeNames            []string `json:"nodeNames,omitempty"`
	NodePrefix           *string  `json:"nodePrefix,omitempty"`
	NodesResizeAvailable *bool    `json:"nodesResizeAvailable,omitempty"`
	// YbaApi Internal. OpenTelemetry Collector enabled for universe
	OtelCollectorEnabled *bool `json:"otelCollectorEnabled,omitempty"`
	// YbaApi Internal. PA Collector UUID
	PaCollectorUuid               *string               `json:"paCollectorUuid,omitempty"`
	PlacementModificationTaskUuid *string               `json:"placementModificationTaskUuid,omitempty"`
	PlatformUrl                   string                `json:"platformUrl"`
	PlatformVersion               *string               `json:"platformVersion,omitempty"`
	PrevYBSoftwareConfig          *PrevYBSoftwareConfig `json:"prevYBSoftwareConfig,omitempty"`
	// Previous task UUID of a retry
	PreviousTaskUUID        *string           `json:"previousTaskUUID,omitempty"`
	RemotePackagePath       *string           `json:"remotePackagePath,omitempty"`
	ResetAZConfig           *bool             `json:"resetAZConfig,omitempty"`
	RollMaxBatchSize        *RollMaxBatchSize `json:"rollMaxBatchSize,omitempty"`
	RootAndClientRootCASame *bool             `json:"rootAndClientRootCASame,omitempty"`
	RootCA                  *string           `json:"rootCA,omitempty"`
	// YbaApi Internal. Run only prechecks during task run
	RunOnlyPrechecks         *bool `json:"runOnlyPrechecks,omitempty"`
	SetTxnTableWaitCountFlag *bool `json:"setTxnTableWaitCountFlag,omitempty"`
	// YbaApi Internal. Whether to skip node prechecks while performing rolling upgrade
	SkipNodeChecks                 *bool   `json:"skipNodeChecks,omitempty"`
	SleepAfterMasterRestartMillis  int32   `json:"sleepAfterMasterRestartMillis"`
	SleepAfterTServerRestartMillis int32   `json:"sleepAfterTServerRestartMillis"`
	SoftwareUpgradeState           *string `json:"softwareUpgradeState,omitempty"`
	// The source universe's xcluster replication relationships
	SourceXClusterConfigs []string `json:"sourceXClusterConfigs,omitempty"`
	SshUserOverride       *string  `json:"sshUserOverride,omitempty"`
	// The target universe's xcluster replication relationships
	TargetXClusterConfigs []string `json:"targetXClusterConfigs,omitempty"`
	// YbaApi Internal. True if a universe has been detached
	UniverseDetached *bool `json:"universeDetached,omitempty"`
	UniversePaused   *bool `json:"universePaused,omitempty"`
	// Associated universe UUID
	UniverseUUID          *string       `json:"universeUUID,omitempty"`
	UpdateInProgress      *bool         `json:"updateInProgress,omitempty"`
	UpdateOptions         []string      `json:"updateOptions,omitempty"`
	UpdateSucceeded       *bool         `json:"updateSucceeded,omitempty"`
	UpdatingTask          *string       `json:"updatingTask,omitempty"`
	UpdatingTaskUUID      *string       `json:"updatingTaskUUID,omitempty"`
	UpgradeOption         string        `json:"upgradeOption"`
	UseNewHelmNamingStyle *bool         `json:"useNewHelmNamingStyle,omitempty"`
	UserAZSelected        *bool         `json:"userAZSelected,omitempty"`
	XclusterInfo          *XClusterInfo `json:"xclusterInfo,omitempty"`
	// Previous software version
	YbPrevSoftwareVersion *string `json:"ybPrevSoftwareVersion,omitempty"`
	YbcInstalled          *bool   `json:"ybcInstalled,omitempty"`
	YbcSoftwareVersion    *string `json:"ybcSoftwareVersion,omitempty"`
}

type _ProvisionUniverseNodesParams ProvisionUniverseNodesParams

// NewProvisionUniverseNodesParams instantiates a new ProvisionUniverseNodesParams object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewProvisionUniverseNodesParams(clusters []Cluster, creatingUser Users, kubernetesUpgradeSupported bool, platformUrl string, sleepAfterMasterRestartMillis int32, sleepAfterTServerRestartMillis int32, upgradeOption string) *ProvisionUniverseNodesParams {
	this := ProvisionUniverseNodesParams{}
	this.Clusters = clusters
	this.CreatingUser = creatingUser
	this.KubernetesUpgradeSupported = kubernetesUpgradeSupported
	this.PlatformUrl = platformUrl
	this.SleepAfterMasterRestartMillis = sleepAfterMasterRestartMillis
	this.SleepAfterTServerRestartMillis = sleepAfterTServerRestartMillis
	this.UpgradeOption = upgradeOption
	return &this
}

// NewProvisionUniverseNodesParamsWithDefaults instantiates a new ProvisionUniverseNodesParams object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewProvisionUniverseNodesParamsWithDefaults() *ProvisionUniverseNodesParams {
	this := ProvisionUniverseNodesParams{}
	return &this
}

// GetAdditionalServicesStateData returns the AdditionalServicesStateData field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetAdditionalServicesStateData() AdditionalServicesStateData {
	if o == nil || IsNil(o.AdditionalServicesStateData) {
		var ret AdditionalServicesStateData
		return ret
	}
	return *o.AdditionalServicesStateData
}

// GetAdditionalServicesStateDataOk returns a tuple with the AdditionalServicesStateData field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetAdditionalServicesStateDataOk() (*AdditionalServicesStateData, bool) {
	if o == nil || IsNil(o.AdditionalServicesStateData) {
		return nil, false
	}
	return o.AdditionalServicesStateData, true
}

// HasAdditionalServicesStateData returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasAdditionalServicesStateData() bool {
	if o != nil && !IsNil(o.AdditionalServicesStateData) {
		return true
	}

	return false
}

// SetAdditionalServicesStateData gets a reference to the given AdditionalServicesStateData and assigns it to the AdditionalServicesStateData field.
func (o *ProvisionUniverseNodesParams) SetAdditionalServicesStateData(v AdditionalServicesStateData) {
	o.AdditionalServicesStateData = &v
}

// GetAllowInsecure returns the AllowInsecure field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetAllowInsecure() bool {
	if o == nil || IsNil(o.AllowInsecure) {
		var ret bool
		return ret
	}
	return *o.AllowInsecure
}

// GetAllowInsecureOk returns a tuple with the AllowInsecure field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetAllowInsecureOk() (*bool, bool) {
	if o == nil || IsNil(o.AllowInsecure) {
		return nil, false
	}
	return o.AllowInsecure, true
}

// HasAllowInsecure returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasAllowInsecure() bool {
	if o != nil && !IsNil(o.AllowInsecure) {
		return true
	}

	return false
}

// SetAllowInsecure gets a reference to the given bool and assigns it to the AllowInsecure field.
func (o *ProvisionUniverseNodesParams) SetAllowInsecure(v bool) {
	o.AllowInsecure = &v
}

// GetArch returns the Arch field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetArch() string {
	if o == nil || IsNil(o.Arch) {
		var ret string
		return ret
	}
	return *o.Arch
}

// GetArchOk returns a tuple with the Arch field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetArchOk() (*string, bool) {
	if o == nil || IsNil(o.Arch) {
		return nil, false
	}
	return o.Arch, true
}

// HasArch returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasArch() bool {
	if o != nil && !IsNil(o.Arch) {
		return true
	}

	return false
}

// SetArch gets a reference to the given string and assigns it to the Arch field.
func (o *ProvisionUniverseNodesParams) SetArch(v string) {
	o.Arch = &v
}

// GetAutoRollbackPerformed returns the AutoRollbackPerformed field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetAutoRollbackPerformed() bool {
	if o == nil || IsNil(o.AutoRollbackPerformed) {
		var ret bool
		return ret
	}
	return *o.AutoRollbackPerformed
}

// GetAutoRollbackPerformedOk returns a tuple with the AutoRollbackPerformed field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetAutoRollbackPerformedOk() (*bool, bool) {
	if o == nil || IsNil(o.AutoRollbackPerformed) {
		return nil, false
	}
	return o.AutoRollbackPerformed, true
}

// HasAutoRollbackPerformed returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasAutoRollbackPerformed() bool {
	if o != nil && !IsNil(o.AutoRollbackPerformed) {
		return true
	}

	return false
}

// SetAutoRollbackPerformed gets a reference to the given bool and assigns it to the AutoRollbackPerformed field.
func (o *ProvisionUniverseNodesParams) SetAutoRollbackPerformed(v bool) {
	o.AutoRollbackPerformed = &v
}

// GetCapability returns the Capability field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetCapability() string {
	if o == nil || IsNil(o.Capability) {
		var ret string
		return ret
	}
	return *o.Capability
}

// GetCapabilityOk returns a tuple with the Capability field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetCapabilityOk() (*string, bool) {
	if o == nil || IsNil(o.Capability) {
		return nil, false
	}
	return o.Capability, true
}

// HasCapability returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasCapability() bool {
	if o != nil && !IsNil(o.Capability) {
		return true
	}

	return false
}

// SetCapability gets a reference to the given string and assigns it to the Capability field.
func (o *ProvisionUniverseNodesParams) SetCapability(v string) {
	o.Capability = &v
}

// GetClientRootCA returns the ClientRootCA field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetClientRootCA() string {
	if o == nil || IsNil(o.ClientRootCA) {
		var ret string
		return ret
	}
	return *o.ClientRootCA
}

// GetClientRootCAOk returns a tuple with the ClientRootCA field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetClientRootCAOk() (*string, bool) {
	if o == nil || IsNil(o.ClientRootCA) {
		return nil, false
	}
	return o.ClientRootCA, true
}

// HasClientRootCA returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasClientRootCA() bool {
	if o != nil && !IsNil(o.ClientRootCA) {
		return true
	}

	return false
}

// SetClientRootCA gets a reference to the given string and assigns it to the ClientRootCA field.
func (o *ProvisionUniverseNodesParams) SetClientRootCA(v string) {
	o.ClientRootCA = &v
}

// GetClusters returns the Clusters field value
func (o *ProvisionUniverseNodesParams) GetClusters() []Cluster {
	if o == nil {
		var ret []Cluster
		return ret
	}

	return o.Clusters
}

// GetClustersOk returns a tuple with the Clusters field value
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetClustersOk() ([]Cluster, bool) {
	if o == nil {
		return nil, false
	}
	return o.Clusters, true
}

// SetClusters sets field value
func (o *ProvisionUniverseNodesParams) SetClusters(v []Cluster) {
	o.Clusters = v
}

// GetCmkArn returns the CmkArn field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetCmkArn() string {
	if o == nil || IsNil(o.CmkArn) {
		var ret string
		return ret
	}
	return *o.CmkArn
}

// GetCmkArnOk returns a tuple with the CmkArn field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetCmkArnOk() (*string, bool) {
	if o == nil || IsNil(o.CmkArn) {
		return nil, false
	}
	return o.CmkArn, true
}

// HasCmkArn returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasCmkArn() bool {
	if o != nil && !IsNil(o.CmkArn) {
		return true
	}

	return false
}

// SetCmkArn gets a reference to the given string and assigns it to the CmkArn field.
func (o *ProvisionUniverseNodesParams) SetCmkArn(v string) {
	o.CmkArn = &v
}

// GetCommunicationPorts returns the CommunicationPorts field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetCommunicationPorts() CommunicationPorts {
	if o == nil || IsNil(o.CommunicationPorts) {
		var ret CommunicationPorts
		return ret
	}
	return *o.CommunicationPorts
}

// GetCommunicationPortsOk returns a tuple with the CommunicationPorts field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetCommunicationPortsOk() (*CommunicationPorts, bool) {
	if o == nil || IsNil(o.CommunicationPorts) {
		return nil, false
	}
	return o.CommunicationPorts, true
}

// HasCommunicationPorts returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasCommunicationPorts() bool {
	if o != nil && !IsNil(o.CommunicationPorts) {
		return true
	}

	return false
}

// SetCommunicationPorts gets a reference to the given CommunicationPorts and assigns it to the CommunicationPorts field.
func (o *ProvisionUniverseNodesParams) SetCommunicationPorts(v CommunicationPorts) {
	o.CommunicationPorts = &v
}

// GetCreatingUser returns the CreatingUser field value
func (o *ProvisionUniverseNodesParams) GetCreatingUser() Users {
	if o == nil {
		var ret Users
		return ret
	}

	return o.CreatingUser
}

// GetCreatingUserOk returns a tuple with the CreatingUser field value
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetCreatingUserOk() (*Users, bool) {
	if o == nil {
		return nil, false
	}
	return &o.CreatingUser, true
}

// SetCreatingUser sets field value
func (o *ProvisionUniverseNodesParams) SetCreatingUser(v Users) {
	o.CreatingUser = v
}

// GetCurrentClusterType returns the CurrentClusterType field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetCurrentClusterType() string {
	if o == nil || IsNil(o.CurrentClusterType) {
		var ret string
		return ret
	}
	return *o.CurrentClusterType
}

// GetCurrentClusterTypeOk returns a tuple with the CurrentClusterType field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetCurrentClusterTypeOk() (*string, bool) {
	if o == nil || IsNil(o.CurrentClusterType) {
		return nil, false
	}
	return o.CurrentClusterType, true
}

// HasCurrentClusterType returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasCurrentClusterType() bool {
	if o != nil && !IsNil(o.CurrentClusterType) {
		return true
	}

	return false
}

// SetCurrentClusterType gets a reference to the given string and assigns it to the CurrentClusterType field.
func (o *ProvisionUniverseNodesParams) SetCurrentClusterType(v string) {
	o.CurrentClusterType = &v
}

// GetDeviceInfo returns the DeviceInfo field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetDeviceInfo() DeviceInfo {
	if o == nil || IsNil(o.DeviceInfo) {
		var ret DeviceInfo
		return ret
	}
	return *o.DeviceInfo
}

// GetDeviceInfoOk returns a tuple with the DeviceInfo field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetDeviceInfoOk() (*DeviceInfo, bool) {
	if o == nil || IsNil(o.DeviceInfo) {
		return nil, false
	}
	return o.DeviceInfo, true
}

// HasDeviceInfo returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasDeviceInfo() bool {
	if o != nil && !IsNil(o.DeviceInfo) {
		return true
	}

	return false
}

// SetDeviceInfo gets a reference to the given DeviceInfo and assigns it to the DeviceInfo field.
func (o *ProvisionUniverseNodesParams) SetDeviceInfo(v DeviceInfo) {
	o.DeviceInfo = &v
}

// GetEnableYbc returns the EnableYbc field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetEnableYbc() bool {
	if o == nil || IsNil(o.EnableYbc) {
		var ret bool
		return ret
	}
	return *o.EnableYbc
}

// GetEnableYbcOk returns a tuple with the EnableYbc field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetEnableYbcOk() (*bool, bool) {
	if o == nil || IsNil(o.EnableYbc) {
		return nil, false
	}
	return o.EnableYbc, true
}

// HasEnableYbc returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasEnableYbc() bool {
	if o != nil && !IsNil(o.EnableYbc) {
		return true
	}

	return false
}

// SetEnableYbc gets a reference to the given bool and assigns it to the EnableYbc field.
func (o *ProvisionUniverseNodesParams) SetEnableYbc(v bool) {
	o.EnableYbc = &v
}

// GetEncryptionAtRestConfig returns the EncryptionAtRestConfig field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetEncryptionAtRestConfig() EncryptionAtRestConfig {
	if o == nil || IsNil(o.EncryptionAtRestConfig) {
		var ret EncryptionAtRestConfig
		return ret
	}
	return *o.EncryptionAtRestConfig
}

// GetEncryptionAtRestConfigOk returns a tuple with the EncryptionAtRestConfig field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetEncryptionAtRestConfigOk() (*EncryptionAtRestConfig, bool) {
	if o == nil || IsNil(o.EncryptionAtRestConfig) {
		return nil, false
	}
	return o.EncryptionAtRestConfig, true
}

// HasEncryptionAtRestConfig returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasEncryptionAtRestConfig() bool {
	if o != nil && !IsNil(o.EncryptionAtRestConfig) {
		return true
	}

	return false
}

// SetEncryptionAtRestConfig gets a reference to the given EncryptionAtRestConfig and assigns it to the EncryptionAtRestConfig field.
func (o *ProvisionUniverseNodesParams) SetEncryptionAtRestConfig(v EncryptionAtRestConfig) {
	o.EncryptionAtRestConfig = &v
}

// GetErrorString returns the ErrorString field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetErrorString() string {
	if o == nil || IsNil(o.ErrorString) {
		var ret string
		return ret
	}
	return *o.ErrorString
}

// GetErrorStringOk returns a tuple with the ErrorString field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetErrorStringOk() (*string, bool) {
	if o == nil || IsNil(o.ErrorString) {
		return nil, false
	}
	return o.ErrorString, true
}

// HasErrorString returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasErrorString() bool {
	if o != nil && !IsNil(o.ErrorString) {
		return true
	}

	return false
}

// SetErrorString gets a reference to the given string and assigns it to the ErrorString field.
func (o *ProvisionUniverseNodesParams) SetErrorString(v string) {
	o.ErrorString = &v
}

// GetExpectedUniverseVersion returns the ExpectedUniverseVersion field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetExpectedUniverseVersion() int32 {
	if o == nil || IsNil(o.ExpectedUniverseVersion) {
		var ret int32
		return ret
	}
	return *o.ExpectedUniverseVersion
}

// GetExpectedUniverseVersionOk returns a tuple with the ExpectedUniverseVersion field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetExpectedUniverseVersionOk() (*int32, bool) {
	if o == nil || IsNil(o.ExpectedUniverseVersion) {
		return nil, false
	}
	return o.ExpectedUniverseVersion, true
}

// HasExpectedUniverseVersion returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasExpectedUniverseVersion() bool {
	if o != nil && !IsNil(o.ExpectedUniverseVersion) {
		return true
	}

	return false
}

// SetExpectedUniverseVersion gets a reference to the given int32 and assigns it to the ExpectedUniverseVersion field.
func (o *ProvisionUniverseNodesParams) SetExpectedUniverseVersion(v int32) {
	o.ExpectedUniverseVersion = &v
}

// GetExtraDependencies returns the ExtraDependencies field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetExtraDependencies() ExtraDependencies {
	if o == nil || IsNil(o.ExtraDependencies) {
		var ret ExtraDependencies
		return ret
	}
	return *o.ExtraDependencies
}

// GetExtraDependenciesOk returns a tuple with the ExtraDependencies field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetExtraDependenciesOk() (*ExtraDependencies, bool) {
	if o == nil || IsNil(o.ExtraDependencies) {
		return nil, false
	}
	return o.ExtraDependencies, true
}

// HasExtraDependencies returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasExtraDependencies() bool {
	if o != nil && !IsNil(o.ExtraDependencies) {
		return true
	}

	return false
}

// SetExtraDependencies gets a reference to the given ExtraDependencies and assigns it to the ExtraDependencies field.
func (o *ProvisionUniverseNodesParams) SetExtraDependencies(v ExtraDependencies) {
	o.ExtraDependencies = &v
}

// GetFipsEnabled returns the FipsEnabled field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetFipsEnabled() bool {
	if o == nil || IsNil(o.FipsEnabled) {
		var ret bool
		return ret
	}
	return *o.FipsEnabled
}

// GetFipsEnabledOk returns a tuple with the FipsEnabled field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetFipsEnabledOk() (*bool, bool) {
	if o == nil || IsNil(o.FipsEnabled) {
		return nil, false
	}
	return o.FipsEnabled, true
}

// HasFipsEnabled returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasFipsEnabled() bool {
	if o != nil && !IsNil(o.FipsEnabled) {
		return true
	}

	return false
}

// SetFipsEnabled gets a reference to the given bool and assigns it to the FipsEnabled field.
func (o *ProvisionUniverseNodesParams) SetFipsEnabled(v bool) {
	o.FipsEnabled = &v
}

// GetImportedState returns the ImportedState field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetImportedState() string {
	if o == nil || IsNil(o.ImportedState) {
		var ret string
		return ret
	}
	return *o.ImportedState
}

// GetImportedStateOk returns a tuple with the ImportedState field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetImportedStateOk() (*string, bool) {
	if o == nil || IsNil(o.ImportedState) {
		return nil, false
	}
	return o.ImportedState, true
}

// HasImportedState returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasImportedState() bool {
	if o != nil && !IsNil(o.ImportedState) {
		return true
	}

	return false
}

// SetImportedState gets a reference to the given string and assigns it to the ImportedState field.
func (o *ProvisionUniverseNodesParams) SetImportedState(v string) {
	o.ImportedState = &v
}

// GetInstallNodeAgent returns the InstallNodeAgent field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetInstallNodeAgent() bool {
	if o == nil || IsNil(o.InstallNodeAgent) {
		var ret bool
		return ret
	}
	return *o.InstallNodeAgent
}

// GetInstallNodeAgentOk returns a tuple with the InstallNodeAgent field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetInstallNodeAgentOk() (*bool, bool) {
	if o == nil || IsNil(o.InstallNodeAgent) {
		return nil, false
	}
	return o.InstallNodeAgent, true
}

// HasInstallNodeAgent returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasInstallNodeAgent() bool {
	if o != nil && !IsNil(o.InstallNodeAgent) {
		return true
	}

	return false
}

// SetInstallNodeAgent gets a reference to the given bool and assigns it to the InstallNodeAgent field.
func (o *ProvisionUniverseNodesParams) SetInstallNodeAgent(v bool) {
	o.InstallNodeAgent = &v
}

// GetInstallYbc returns the InstallYbc field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetInstallYbc() bool {
	if o == nil || IsNil(o.InstallYbc) {
		var ret bool
		return ret
	}
	return *o.InstallYbc
}

// GetInstallYbcOk returns a tuple with the InstallYbc field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetInstallYbcOk() (*bool, bool) {
	if o == nil || IsNil(o.InstallYbc) {
		return nil, false
	}
	return o.InstallYbc, true
}

// HasInstallYbc returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasInstallYbc() bool {
	if o != nil && !IsNil(o.InstallYbc) {
		return true
	}

	return false
}

// SetInstallYbc gets a reference to the given bool and assigns it to the InstallYbc field.
func (o *ProvisionUniverseNodesParams) SetInstallYbc(v bool) {
	o.InstallYbc = &v
}

// GetIsKubernetesOperatorControlled returns the IsKubernetesOperatorControlled field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetIsKubernetesOperatorControlled() bool {
	if o == nil || IsNil(o.IsKubernetesOperatorControlled) {
		var ret bool
		return ret
	}
	return *o.IsKubernetesOperatorControlled
}

// GetIsKubernetesOperatorControlledOk returns a tuple with the IsKubernetesOperatorControlled field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetIsKubernetesOperatorControlledOk() (*bool, bool) {
	if o == nil || IsNil(o.IsKubernetesOperatorControlled) {
		return nil, false
	}
	return o.IsKubernetesOperatorControlled, true
}

// HasIsKubernetesOperatorControlled returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasIsKubernetesOperatorControlled() bool {
	if o != nil && !IsNil(o.IsKubernetesOperatorControlled) {
		return true
	}

	return false
}

// SetIsKubernetesOperatorControlled gets a reference to the given bool and assigns it to the IsKubernetesOperatorControlled field.
func (o *ProvisionUniverseNodesParams) SetIsKubernetesOperatorControlled(v bool) {
	o.IsKubernetesOperatorControlled = &v
}

// GetIsSoftwareRollbackAllowed returns the IsSoftwareRollbackAllowed field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetIsSoftwareRollbackAllowed() bool {
	if o == nil || IsNil(o.IsSoftwareRollbackAllowed) {
		var ret bool
		return ret
	}
	return *o.IsSoftwareRollbackAllowed
}

// GetIsSoftwareRollbackAllowedOk returns a tuple with the IsSoftwareRollbackAllowed field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetIsSoftwareRollbackAllowedOk() (*bool, bool) {
	if o == nil || IsNil(o.IsSoftwareRollbackAllowed) {
		return nil, false
	}
	return o.IsSoftwareRollbackAllowed, true
}

// HasIsSoftwareRollbackAllowed returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasIsSoftwareRollbackAllowed() bool {
	if o != nil && !IsNil(o.IsSoftwareRollbackAllowed) {
		return true
	}

	return false
}

// SetIsSoftwareRollbackAllowed gets a reference to the given bool and assigns it to the IsSoftwareRollbackAllowed field.
func (o *ProvisionUniverseNodesParams) SetIsSoftwareRollbackAllowed(v bool) {
	o.IsSoftwareRollbackAllowed = &v
}

// GetItestS3PackagePath returns the ItestS3PackagePath field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetItestS3PackagePath() string {
	if o == nil || IsNil(o.ItestS3PackagePath) {
		var ret string
		return ret
	}
	return *o.ItestS3PackagePath
}

// GetItestS3PackagePathOk returns a tuple with the ItestS3PackagePath field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetItestS3PackagePathOk() (*string, bool) {
	if o == nil || IsNil(o.ItestS3PackagePath) {
		return nil, false
	}
	return o.ItestS3PackagePath, true
}

// HasItestS3PackagePath returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasItestS3PackagePath() bool {
	if o != nil && !IsNil(o.ItestS3PackagePath) {
		return true
	}

	return false
}

// SetItestS3PackagePath gets a reference to the given string and assigns it to the ItestS3PackagePath field.
func (o *ProvisionUniverseNodesParams) SetItestS3PackagePath(v string) {
	o.ItestS3PackagePath = &v
}

// GetKubernetesUpgradeSupported returns the KubernetesUpgradeSupported field value
func (o *ProvisionUniverseNodesParams) GetKubernetesUpgradeSupported() bool {
	if o == nil {
		var ret bool
		return ret
	}

	return o.KubernetesUpgradeSupported
}

// GetKubernetesUpgradeSupportedOk returns a tuple with the KubernetesUpgradeSupported field value
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetKubernetesUpgradeSupportedOk() (*bool, bool) {
	if o == nil {
		return nil, false
	}
	return &o.KubernetesUpgradeSupported, true
}

// SetKubernetesUpgradeSupported sets field value
func (o *ProvisionUniverseNodesParams) SetKubernetesUpgradeSupported(v bool) {
	o.KubernetesUpgradeSupported = v
}

// GetMastersInDefaultRegion returns the MastersInDefaultRegion field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetMastersInDefaultRegion() bool {
	if o == nil || IsNil(o.MastersInDefaultRegion) {
		var ret bool
		return ret
	}
	return *o.MastersInDefaultRegion
}

// GetMastersInDefaultRegionOk returns a tuple with the MastersInDefaultRegion field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetMastersInDefaultRegionOk() (*bool, bool) {
	if o == nil || IsNil(o.MastersInDefaultRegion) {
		return nil, false
	}
	return o.MastersInDefaultRegion, true
}

// HasMastersInDefaultRegion returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasMastersInDefaultRegion() bool {
	if o != nil && !IsNil(o.MastersInDefaultRegion) {
		return true
	}

	return false
}

// SetMastersInDefaultRegion gets a reference to the given bool and assigns it to the MastersInDefaultRegion field.
func (o *ProvisionUniverseNodesParams) SetMastersInDefaultRegion(v bool) {
	o.MastersInDefaultRegion = &v
}

// GetNextClusterIndex returns the NextClusterIndex field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetNextClusterIndex() int32 {
	if o == nil || IsNil(o.NextClusterIndex) {
		var ret int32
		return ret
	}
	return *o.NextClusterIndex
}

// GetNextClusterIndexOk returns a tuple with the NextClusterIndex field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetNextClusterIndexOk() (*int32, bool) {
	if o == nil || IsNil(o.NextClusterIndex) {
		return nil, false
	}
	return o.NextClusterIndex, true
}

// HasNextClusterIndex returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasNextClusterIndex() bool {
	if o != nil && !IsNil(o.NextClusterIndex) {
		return true
	}

	return false
}

// SetNextClusterIndex gets a reference to the given int32 and assigns it to the NextClusterIndex field.
func (o *ProvisionUniverseNodesParams) SetNextClusterIndex(v int32) {
	o.NextClusterIndex = &v
}

// GetNodeAgentMissing returns the NodeAgentMissing field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetNodeAgentMissing() bool {
	if o == nil || IsNil(o.NodeAgentMissing) {
		var ret bool
		return ret
	}
	return *o.NodeAgentMissing
}

// GetNodeAgentMissingOk returns a tuple with the NodeAgentMissing field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetNodeAgentMissingOk() (*bool, bool) {
	if o == nil || IsNil(o.NodeAgentMissing) {
		return nil, false
	}
	return o.NodeAgentMissing, true
}

// HasNodeAgentMissing returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasNodeAgentMissing() bool {
	if o != nil && !IsNil(o.NodeAgentMissing) {
		return true
	}

	return false
}

// SetNodeAgentMissing gets a reference to the given bool and assigns it to the NodeAgentMissing field.
func (o *ProvisionUniverseNodesParams) SetNodeAgentMissing(v bool) {
	o.NodeAgentMissing = &v
}

// GetNodeDetailsSet returns the NodeDetailsSet field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetNodeDetailsSet() []NodeDetails {
	if o == nil || IsNil(o.NodeDetailsSet) {
		var ret []NodeDetails
		return ret
	}
	return o.NodeDetailsSet
}

// GetNodeDetailsSetOk returns a tuple with the NodeDetailsSet field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetNodeDetailsSetOk() ([]NodeDetails, bool) {
	if o == nil || IsNil(o.NodeDetailsSet) {
		return nil, false
	}
	return o.NodeDetailsSet, true
}

// HasNodeDetailsSet returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasNodeDetailsSet() bool {
	if o != nil && !IsNil(o.NodeDetailsSet) {
		return true
	}

	return false
}

// SetNodeDetailsSet gets a reference to the given []NodeDetails and assigns it to the NodeDetailsSet field.
func (o *ProvisionUniverseNodesParams) SetNodeDetailsSet(v []NodeDetails) {
	o.NodeDetailsSet = v
}

// GetNodeExporterUser returns the NodeExporterUser field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetNodeExporterUser() string {
	if o == nil || IsNil(o.NodeExporterUser) {
		var ret string
		return ret
	}
	return *o.NodeExporterUser
}

// GetNodeExporterUserOk returns a tuple with the NodeExporterUser field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetNodeExporterUserOk() (*string, bool) {
	if o == nil || IsNil(o.NodeExporterUser) {
		return nil, false
	}
	return o.NodeExporterUser, true
}

// HasNodeExporterUser returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasNodeExporterUser() bool {
	if o != nil && !IsNil(o.NodeExporterUser) {
		return true
	}

	return false
}

// SetNodeExporterUser gets a reference to the given string and assigns it to the NodeExporterUser field.
func (o *ProvisionUniverseNodesParams) SetNodeExporterUser(v string) {
	o.NodeExporterUser = &v
}

// GetNodeNames returns the NodeNames field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetNodeNames() []string {
	if o == nil || IsNil(o.NodeNames) {
		var ret []string
		return ret
	}
	return o.NodeNames
}

// GetNodeNamesOk returns a tuple with the NodeNames field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetNodeNamesOk() ([]string, bool) {
	if o == nil || IsNil(o.NodeNames) {
		return nil, false
	}
	return o.NodeNames, true
}

// HasNodeNames returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasNodeNames() bool {
	if o != nil && !IsNil(o.NodeNames) {
		return true
	}

	return false
}

// SetNodeNames gets a reference to the given []string and assigns it to the NodeNames field.
func (o *ProvisionUniverseNodesParams) SetNodeNames(v []string) {
	o.NodeNames = v
}

// GetNodePrefix returns the NodePrefix field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetNodePrefix() string {
	if o == nil || IsNil(o.NodePrefix) {
		var ret string
		return ret
	}
	return *o.NodePrefix
}

// GetNodePrefixOk returns a tuple with the NodePrefix field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetNodePrefixOk() (*string, bool) {
	if o == nil || IsNil(o.NodePrefix) {
		return nil, false
	}
	return o.NodePrefix, true
}

// HasNodePrefix returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasNodePrefix() bool {
	if o != nil && !IsNil(o.NodePrefix) {
		return true
	}

	return false
}

// SetNodePrefix gets a reference to the given string and assigns it to the NodePrefix field.
func (o *ProvisionUniverseNodesParams) SetNodePrefix(v string) {
	o.NodePrefix = &v
}

// GetNodesResizeAvailable returns the NodesResizeAvailable field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetNodesResizeAvailable() bool {
	if o == nil || IsNil(o.NodesResizeAvailable) {
		var ret bool
		return ret
	}
	return *o.NodesResizeAvailable
}

// GetNodesResizeAvailableOk returns a tuple with the NodesResizeAvailable field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetNodesResizeAvailableOk() (*bool, bool) {
	if o == nil || IsNil(o.NodesResizeAvailable) {
		return nil, false
	}
	return o.NodesResizeAvailable, true
}

// HasNodesResizeAvailable returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasNodesResizeAvailable() bool {
	if o != nil && !IsNil(o.NodesResizeAvailable) {
		return true
	}

	return false
}

// SetNodesResizeAvailable gets a reference to the given bool and assigns it to the NodesResizeAvailable field.
func (o *ProvisionUniverseNodesParams) SetNodesResizeAvailable(v bool) {
	o.NodesResizeAvailable = &v
}

// GetOtelCollectorEnabled returns the OtelCollectorEnabled field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetOtelCollectorEnabled() bool {
	if o == nil || IsNil(o.OtelCollectorEnabled) {
		var ret bool
		return ret
	}
	return *o.OtelCollectorEnabled
}

// GetOtelCollectorEnabledOk returns a tuple with the OtelCollectorEnabled field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetOtelCollectorEnabledOk() (*bool, bool) {
	if o == nil || IsNil(o.OtelCollectorEnabled) {
		return nil, false
	}
	return o.OtelCollectorEnabled, true
}

// HasOtelCollectorEnabled returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasOtelCollectorEnabled() bool {
	if o != nil && !IsNil(o.OtelCollectorEnabled) {
		return true
	}

	return false
}

// SetOtelCollectorEnabled gets a reference to the given bool and assigns it to the OtelCollectorEnabled field.
func (o *ProvisionUniverseNodesParams) SetOtelCollectorEnabled(v bool) {
	o.OtelCollectorEnabled = &v
}

// GetPaCollectorUuid returns the PaCollectorUuid field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetPaCollectorUuid() string {
	if o == nil || IsNil(o.PaCollectorUuid) {
		var ret string
		return ret
	}
	return *o.PaCollectorUuid
}

// GetPaCollectorUuidOk returns a tuple with the PaCollectorUuid field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetPaCollectorUuidOk() (*string, bool) {
	if o == nil || IsNil(o.PaCollectorUuid) {
		return nil, false
	}
	return o.PaCollectorUuid, true
}

// HasPaCollectorUuid returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasPaCollectorUuid() bool {
	if o != nil && !IsNil(o.PaCollectorUuid) {
		return true
	}

	return false
}

// SetPaCollectorUuid gets a reference to the given string and assigns it to the PaCollectorUuid field.
func (o *ProvisionUniverseNodesParams) SetPaCollectorUuid(v string) {
	o.PaCollectorUuid = &v
}

// GetPlacementModificationTaskUuid returns the PlacementModificationTaskUuid field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetPlacementModificationTaskUuid() string {
	if o == nil || IsNil(o.PlacementModificationTaskUuid) {
		var ret string
		return ret
	}
	return *o.PlacementModificationTaskUuid
}

// GetPlacementModificationTaskUuidOk returns a tuple with the PlacementModificationTaskUuid field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetPlacementModificationTaskUuidOk() (*string, bool) {
	if o == nil || IsNil(o.PlacementModificationTaskUuid) {
		return nil, false
	}
	return o.PlacementModificationTaskUuid, true
}

// HasPlacementModificationTaskUuid returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasPlacementModificationTaskUuid() bool {
	if o != nil && !IsNil(o.PlacementModificationTaskUuid) {
		return true
	}

	return false
}

// SetPlacementModificationTaskUuid gets a reference to the given string and assigns it to the PlacementModificationTaskUuid field.
func (o *ProvisionUniverseNodesParams) SetPlacementModificationTaskUuid(v string) {
	o.PlacementModificationTaskUuid = &v
}

// GetPlatformUrl returns the PlatformUrl field value
func (o *ProvisionUniverseNodesParams) GetPlatformUrl() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.PlatformUrl
}

// GetPlatformUrlOk returns a tuple with the PlatformUrl field value
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetPlatformUrlOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.PlatformUrl, true
}

// SetPlatformUrl sets field value
func (o *ProvisionUniverseNodesParams) SetPlatformUrl(v string) {
	o.PlatformUrl = v
}

// GetPlatformVersion returns the PlatformVersion field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetPlatformVersion() string {
	if o == nil || IsNil(o.PlatformVersion) {
		var ret string
		return ret
	}
	return *o.PlatformVersion
}

// GetPlatformVersionOk returns a tuple with the PlatformVersion field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetPlatformVersionOk() (*string, bool) {
	if o == nil || IsNil(o.PlatformVersion) {
		return nil, false
	}
	return o.PlatformVersion, true
}

// HasPlatformVersion returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasPlatformVersion() bool {
	if o != nil && !IsNil(o.PlatformVersion) {
		return true
	}

	return false
}

// SetPlatformVersion gets a reference to the given string and assigns it to the PlatformVersion field.
func (o *ProvisionUniverseNodesParams) SetPlatformVersion(v string) {
	o.PlatformVersion = &v
}

// GetPrevYBSoftwareConfig returns the PrevYBSoftwareConfig field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetPrevYBSoftwareConfig() PrevYBSoftwareConfig {
	if o == nil || IsNil(o.PrevYBSoftwareConfig) {
		var ret PrevYBSoftwareConfig
		return ret
	}
	return *o.PrevYBSoftwareConfig
}

// GetPrevYBSoftwareConfigOk returns a tuple with the PrevYBSoftwareConfig field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetPrevYBSoftwareConfigOk() (*PrevYBSoftwareConfig, bool) {
	if o == nil || IsNil(o.PrevYBSoftwareConfig) {
		return nil, false
	}
	return o.PrevYBSoftwareConfig, true
}

// HasPrevYBSoftwareConfig returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasPrevYBSoftwareConfig() bool {
	if o != nil && !IsNil(o.PrevYBSoftwareConfig) {
		return true
	}

	return false
}

// SetPrevYBSoftwareConfig gets a reference to the given PrevYBSoftwareConfig and assigns it to the PrevYBSoftwareConfig field.
func (o *ProvisionUniverseNodesParams) SetPrevYBSoftwareConfig(v PrevYBSoftwareConfig) {
	o.PrevYBSoftwareConfig = &v
}

// GetPreviousTaskUUID returns the PreviousTaskUUID field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetPreviousTaskUUID() string {
	if o == nil || IsNil(o.PreviousTaskUUID) {
		var ret string
		return ret
	}
	return *o.PreviousTaskUUID
}

// GetPreviousTaskUUIDOk returns a tuple with the PreviousTaskUUID field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetPreviousTaskUUIDOk() (*string, bool) {
	if o == nil || IsNil(o.PreviousTaskUUID) {
		return nil, false
	}
	return o.PreviousTaskUUID, true
}

// HasPreviousTaskUUID returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasPreviousTaskUUID() bool {
	if o != nil && !IsNil(o.PreviousTaskUUID) {
		return true
	}

	return false
}

// SetPreviousTaskUUID gets a reference to the given string and assigns it to the PreviousTaskUUID field.
func (o *ProvisionUniverseNodesParams) SetPreviousTaskUUID(v string) {
	o.PreviousTaskUUID = &v
}

// GetRemotePackagePath returns the RemotePackagePath field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetRemotePackagePath() string {
	if o == nil || IsNil(o.RemotePackagePath) {
		var ret string
		return ret
	}
	return *o.RemotePackagePath
}

// GetRemotePackagePathOk returns a tuple with the RemotePackagePath field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetRemotePackagePathOk() (*string, bool) {
	if o == nil || IsNil(o.RemotePackagePath) {
		return nil, false
	}
	return o.RemotePackagePath, true
}

// HasRemotePackagePath returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasRemotePackagePath() bool {
	if o != nil && !IsNil(o.RemotePackagePath) {
		return true
	}

	return false
}

// SetRemotePackagePath gets a reference to the given string and assigns it to the RemotePackagePath field.
func (o *ProvisionUniverseNodesParams) SetRemotePackagePath(v string) {
	o.RemotePackagePath = &v
}

// GetResetAZConfig returns the ResetAZConfig field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetResetAZConfig() bool {
	if o == nil || IsNil(o.ResetAZConfig) {
		var ret bool
		return ret
	}
	return *o.ResetAZConfig
}

// GetResetAZConfigOk returns a tuple with the ResetAZConfig field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetResetAZConfigOk() (*bool, bool) {
	if o == nil || IsNil(o.ResetAZConfig) {
		return nil, false
	}
	return o.ResetAZConfig, true
}

// HasResetAZConfig returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasResetAZConfig() bool {
	if o != nil && !IsNil(o.ResetAZConfig) {
		return true
	}

	return false
}

// SetResetAZConfig gets a reference to the given bool and assigns it to the ResetAZConfig field.
func (o *ProvisionUniverseNodesParams) SetResetAZConfig(v bool) {
	o.ResetAZConfig = &v
}

// GetRollMaxBatchSize returns the RollMaxBatchSize field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetRollMaxBatchSize() RollMaxBatchSize {
	if o == nil || IsNil(o.RollMaxBatchSize) {
		var ret RollMaxBatchSize
		return ret
	}
	return *o.RollMaxBatchSize
}

// GetRollMaxBatchSizeOk returns a tuple with the RollMaxBatchSize field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetRollMaxBatchSizeOk() (*RollMaxBatchSize, bool) {
	if o == nil || IsNil(o.RollMaxBatchSize) {
		return nil, false
	}
	return o.RollMaxBatchSize, true
}

// HasRollMaxBatchSize returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasRollMaxBatchSize() bool {
	if o != nil && !IsNil(o.RollMaxBatchSize) {
		return true
	}

	return false
}

// SetRollMaxBatchSize gets a reference to the given RollMaxBatchSize and assigns it to the RollMaxBatchSize field.
func (o *ProvisionUniverseNodesParams) SetRollMaxBatchSize(v RollMaxBatchSize) {
	o.RollMaxBatchSize = &v
}

// GetRootAndClientRootCASame returns the RootAndClientRootCASame field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetRootAndClientRootCASame() bool {
	if o == nil || IsNil(o.RootAndClientRootCASame) {
		var ret bool
		return ret
	}
	return *o.RootAndClientRootCASame
}

// GetRootAndClientRootCASameOk returns a tuple with the RootAndClientRootCASame field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetRootAndClientRootCASameOk() (*bool, bool) {
	if o == nil || IsNil(o.RootAndClientRootCASame) {
		return nil, false
	}
	return o.RootAndClientRootCASame, true
}

// HasRootAndClientRootCASame returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasRootAndClientRootCASame() bool {
	if o != nil && !IsNil(o.RootAndClientRootCASame) {
		return true
	}

	return false
}

// SetRootAndClientRootCASame gets a reference to the given bool and assigns it to the RootAndClientRootCASame field.
func (o *ProvisionUniverseNodesParams) SetRootAndClientRootCASame(v bool) {
	o.RootAndClientRootCASame = &v
}

// GetRootCA returns the RootCA field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetRootCA() string {
	if o == nil || IsNil(o.RootCA) {
		var ret string
		return ret
	}
	return *o.RootCA
}

// GetRootCAOk returns a tuple with the RootCA field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetRootCAOk() (*string, bool) {
	if o == nil || IsNil(o.RootCA) {
		return nil, false
	}
	return o.RootCA, true
}

// HasRootCA returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasRootCA() bool {
	if o != nil && !IsNil(o.RootCA) {
		return true
	}

	return false
}

// SetRootCA gets a reference to the given string and assigns it to the RootCA field.
func (o *ProvisionUniverseNodesParams) SetRootCA(v string) {
	o.RootCA = &v
}

// GetRunOnlyPrechecks returns the RunOnlyPrechecks field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetRunOnlyPrechecks() bool {
	if o == nil || IsNil(o.RunOnlyPrechecks) {
		var ret bool
		return ret
	}
	return *o.RunOnlyPrechecks
}

// GetRunOnlyPrechecksOk returns a tuple with the RunOnlyPrechecks field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetRunOnlyPrechecksOk() (*bool, bool) {
	if o == nil || IsNil(o.RunOnlyPrechecks) {
		return nil, false
	}
	return o.RunOnlyPrechecks, true
}

// HasRunOnlyPrechecks returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasRunOnlyPrechecks() bool {
	if o != nil && !IsNil(o.RunOnlyPrechecks) {
		return true
	}

	return false
}

// SetRunOnlyPrechecks gets a reference to the given bool and assigns it to the RunOnlyPrechecks field.
func (o *ProvisionUniverseNodesParams) SetRunOnlyPrechecks(v bool) {
	o.RunOnlyPrechecks = &v
}

// GetSetTxnTableWaitCountFlag returns the SetTxnTableWaitCountFlag field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetSetTxnTableWaitCountFlag() bool {
	if o == nil || IsNil(o.SetTxnTableWaitCountFlag) {
		var ret bool
		return ret
	}
	return *o.SetTxnTableWaitCountFlag
}

// GetSetTxnTableWaitCountFlagOk returns a tuple with the SetTxnTableWaitCountFlag field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetSetTxnTableWaitCountFlagOk() (*bool, bool) {
	if o == nil || IsNil(o.SetTxnTableWaitCountFlag) {
		return nil, false
	}
	return o.SetTxnTableWaitCountFlag, true
}

// HasSetTxnTableWaitCountFlag returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasSetTxnTableWaitCountFlag() bool {
	if o != nil && !IsNil(o.SetTxnTableWaitCountFlag) {
		return true
	}

	return false
}

// SetSetTxnTableWaitCountFlag gets a reference to the given bool and assigns it to the SetTxnTableWaitCountFlag field.
func (o *ProvisionUniverseNodesParams) SetSetTxnTableWaitCountFlag(v bool) {
	o.SetTxnTableWaitCountFlag = &v
}

// GetSkipNodeChecks returns the SkipNodeChecks field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetSkipNodeChecks() bool {
	if o == nil || IsNil(o.SkipNodeChecks) {
		var ret bool
		return ret
	}
	return *o.SkipNodeChecks
}

// GetSkipNodeChecksOk returns a tuple with the SkipNodeChecks field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetSkipNodeChecksOk() (*bool, bool) {
	if o == nil || IsNil(o.SkipNodeChecks) {
		return nil, false
	}
	return o.SkipNodeChecks, true
}

// HasSkipNodeChecks returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasSkipNodeChecks() bool {
	if o != nil && !IsNil(o.SkipNodeChecks) {
		return true
	}

	return false
}

// SetSkipNodeChecks gets a reference to the given bool and assigns it to the SkipNodeChecks field.
func (o *ProvisionUniverseNodesParams) SetSkipNodeChecks(v bool) {
	o.SkipNodeChecks = &v
}

// GetSleepAfterMasterRestartMillis returns the SleepAfterMasterRestartMillis field value
func (o *ProvisionUniverseNodesParams) GetSleepAfterMasterRestartMillis() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.SleepAfterMasterRestartMillis
}

// GetSleepAfterMasterRestartMillisOk returns a tuple with the SleepAfterMasterRestartMillis field value
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetSleepAfterMasterRestartMillisOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.SleepAfterMasterRestartMillis, true
}

// SetSleepAfterMasterRestartMillis sets field value
func (o *ProvisionUniverseNodesParams) SetSleepAfterMasterRestartMillis(v int32) {
	o.SleepAfterMasterRestartMillis = v
}

// GetSleepAfterTServerRestartMillis returns the SleepAfterTServerRestartMillis field value
func (o *ProvisionUniverseNodesParams) GetSleepAfterTServerRestartMillis() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.SleepAfterTServerRestartMillis
}

// GetSleepAfterTServerRestartMillisOk returns a tuple with the SleepAfterTServerRestartMillis field value
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetSleepAfterTServerRestartMillisOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.SleepAfterTServerRestartMillis, true
}

// SetSleepAfterTServerRestartMillis sets field value
func (o *ProvisionUniverseNodesParams) SetSleepAfterTServerRestartMillis(v int32) {
	o.SleepAfterTServerRestartMillis = v
}

// GetSoftwareUpgradeState returns the SoftwareUpgradeState field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetSoftwareUpgradeState() string {
	if o == nil || IsNil(o.SoftwareUpgradeState) {
		var ret string
		return ret
	}
	return *o.SoftwareUpgradeState
}

// GetSoftwareUpgradeStateOk returns a tuple with the SoftwareUpgradeState field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetSoftwareUpgradeStateOk() (*string, bool) {
	if o == nil || IsNil(o.SoftwareUpgradeState) {
		return nil, false
	}
	return o.SoftwareUpgradeState, true
}

// HasSoftwareUpgradeState returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasSoftwareUpgradeState() bool {
	if o != nil && !IsNil(o.SoftwareUpgradeState) {
		return true
	}

	return false
}

// SetSoftwareUpgradeState gets a reference to the given string and assigns it to the SoftwareUpgradeState field.
func (o *ProvisionUniverseNodesParams) SetSoftwareUpgradeState(v string) {
	o.SoftwareUpgradeState = &v
}

// GetSourceXClusterConfigs returns the SourceXClusterConfigs field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetSourceXClusterConfigs() []string {
	if o == nil || IsNil(o.SourceXClusterConfigs) {
		var ret []string
		return ret
	}
	return o.SourceXClusterConfigs
}

// GetSourceXClusterConfigsOk returns a tuple with the SourceXClusterConfigs field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetSourceXClusterConfigsOk() ([]string, bool) {
	if o == nil || IsNil(o.SourceXClusterConfigs) {
		return nil, false
	}
	return o.SourceXClusterConfigs, true
}

// HasSourceXClusterConfigs returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasSourceXClusterConfigs() bool {
	if o != nil && !IsNil(o.SourceXClusterConfigs) {
		return true
	}

	return false
}

// SetSourceXClusterConfigs gets a reference to the given []string and assigns it to the SourceXClusterConfigs field.
func (o *ProvisionUniverseNodesParams) SetSourceXClusterConfigs(v []string) {
	o.SourceXClusterConfigs = v
}

// GetSshUserOverride returns the SshUserOverride field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetSshUserOverride() string {
	if o == nil || IsNil(o.SshUserOverride) {
		var ret string
		return ret
	}
	return *o.SshUserOverride
}

// GetSshUserOverrideOk returns a tuple with the SshUserOverride field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetSshUserOverrideOk() (*string, bool) {
	if o == nil || IsNil(o.SshUserOverride) {
		return nil, false
	}
	return o.SshUserOverride, true
}

// HasSshUserOverride returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasSshUserOverride() bool {
	if o != nil && !IsNil(o.SshUserOverride) {
		return true
	}

	return false
}

// SetSshUserOverride gets a reference to the given string and assigns it to the SshUserOverride field.
func (o *ProvisionUniverseNodesParams) SetSshUserOverride(v string) {
	o.SshUserOverride = &v
}

// GetTargetXClusterConfigs returns the TargetXClusterConfigs field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetTargetXClusterConfigs() []string {
	if o == nil || IsNil(o.TargetXClusterConfigs) {
		var ret []string
		return ret
	}
	return o.TargetXClusterConfigs
}

// GetTargetXClusterConfigsOk returns a tuple with the TargetXClusterConfigs field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetTargetXClusterConfigsOk() ([]string, bool) {
	if o == nil || IsNil(o.TargetXClusterConfigs) {
		return nil, false
	}
	return o.TargetXClusterConfigs, true
}

// HasTargetXClusterConfigs returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasTargetXClusterConfigs() bool {
	if o != nil && !IsNil(o.TargetXClusterConfigs) {
		return true
	}

	return false
}

// SetTargetXClusterConfigs gets a reference to the given []string and assigns it to the TargetXClusterConfigs field.
func (o *ProvisionUniverseNodesParams) SetTargetXClusterConfigs(v []string) {
	o.TargetXClusterConfigs = v
}

// GetUniverseDetached returns the UniverseDetached field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetUniverseDetached() bool {
	if o == nil || IsNil(o.UniverseDetached) {
		var ret bool
		return ret
	}
	return *o.UniverseDetached
}

// GetUniverseDetachedOk returns a tuple with the UniverseDetached field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetUniverseDetachedOk() (*bool, bool) {
	if o == nil || IsNil(o.UniverseDetached) {
		return nil, false
	}
	return o.UniverseDetached, true
}

// HasUniverseDetached returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasUniverseDetached() bool {
	if o != nil && !IsNil(o.UniverseDetached) {
		return true
	}

	return false
}

// SetUniverseDetached gets a reference to the given bool and assigns it to the UniverseDetached field.
func (o *ProvisionUniverseNodesParams) SetUniverseDetached(v bool) {
	o.UniverseDetached = &v
}

// GetUniversePaused returns the UniversePaused field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetUniversePaused() bool {
	if o == nil || IsNil(o.UniversePaused) {
		var ret bool
		return ret
	}
	return *o.UniversePaused
}

// GetUniversePausedOk returns a tuple with the UniversePaused field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetUniversePausedOk() (*bool, bool) {
	if o == nil || IsNil(o.UniversePaused) {
		return nil, false
	}
	return o.UniversePaused, true
}

// HasUniversePaused returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasUniversePaused() bool {
	if o != nil && !IsNil(o.UniversePaused) {
		return true
	}

	return false
}

// SetUniversePaused gets a reference to the given bool and assigns it to the UniversePaused field.
func (o *ProvisionUniverseNodesParams) SetUniversePaused(v bool) {
	o.UniversePaused = &v
}

// GetUniverseUUID returns the UniverseUUID field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetUniverseUUID() string {
	if o == nil || IsNil(o.UniverseUUID) {
		var ret string
		return ret
	}
	return *o.UniverseUUID
}

// GetUniverseUUIDOk returns a tuple with the UniverseUUID field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetUniverseUUIDOk() (*string, bool) {
	if o == nil || IsNil(o.UniverseUUID) {
		return nil, false
	}
	return o.UniverseUUID, true
}

// HasUniverseUUID returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasUniverseUUID() bool {
	if o != nil && !IsNil(o.UniverseUUID) {
		return true
	}

	return false
}

// SetUniverseUUID gets a reference to the given string and assigns it to the UniverseUUID field.
func (o *ProvisionUniverseNodesParams) SetUniverseUUID(v string) {
	o.UniverseUUID = &v
}

// GetUpdateInProgress returns the UpdateInProgress field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetUpdateInProgress() bool {
	if o == nil || IsNil(o.UpdateInProgress) {
		var ret bool
		return ret
	}
	return *o.UpdateInProgress
}

// GetUpdateInProgressOk returns a tuple with the UpdateInProgress field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetUpdateInProgressOk() (*bool, bool) {
	if o == nil || IsNil(o.UpdateInProgress) {
		return nil, false
	}
	return o.UpdateInProgress, true
}

// HasUpdateInProgress returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasUpdateInProgress() bool {
	if o != nil && !IsNil(o.UpdateInProgress) {
		return true
	}

	return false
}

// SetUpdateInProgress gets a reference to the given bool and assigns it to the UpdateInProgress field.
func (o *ProvisionUniverseNodesParams) SetUpdateInProgress(v bool) {
	o.UpdateInProgress = &v
}

// GetUpdateOptions returns the UpdateOptions field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetUpdateOptions() []string {
	if o == nil || IsNil(o.UpdateOptions) {
		var ret []string
		return ret
	}
	return o.UpdateOptions
}

// GetUpdateOptionsOk returns a tuple with the UpdateOptions field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetUpdateOptionsOk() ([]string, bool) {
	if o == nil || IsNil(o.UpdateOptions) {
		return nil, false
	}
	return o.UpdateOptions, true
}

// HasUpdateOptions returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasUpdateOptions() bool {
	if o != nil && !IsNil(o.UpdateOptions) {
		return true
	}

	return false
}

// SetUpdateOptions gets a reference to the given []string and assigns it to the UpdateOptions field.
func (o *ProvisionUniverseNodesParams) SetUpdateOptions(v []string) {
	o.UpdateOptions = v
}

// GetUpdateSucceeded returns the UpdateSucceeded field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetUpdateSucceeded() bool {
	if o == nil || IsNil(o.UpdateSucceeded) {
		var ret bool
		return ret
	}
	return *o.UpdateSucceeded
}

// GetUpdateSucceededOk returns a tuple with the UpdateSucceeded field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetUpdateSucceededOk() (*bool, bool) {
	if o == nil || IsNil(o.UpdateSucceeded) {
		return nil, false
	}
	return o.UpdateSucceeded, true
}

// HasUpdateSucceeded returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasUpdateSucceeded() bool {
	if o != nil && !IsNil(o.UpdateSucceeded) {
		return true
	}

	return false
}

// SetUpdateSucceeded gets a reference to the given bool and assigns it to the UpdateSucceeded field.
func (o *ProvisionUniverseNodesParams) SetUpdateSucceeded(v bool) {
	o.UpdateSucceeded = &v
}

// GetUpdatingTask returns the UpdatingTask field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetUpdatingTask() string {
	if o == nil || IsNil(o.UpdatingTask) {
		var ret string
		return ret
	}
	return *o.UpdatingTask
}

// GetUpdatingTaskOk returns a tuple with the UpdatingTask field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetUpdatingTaskOk() (*string, bool) {
	if o == nil || IsNil(o.UpdatingTask) {
		return nil, false
	}
	return o.UpdatingTask, true
}

// HasUpdatingTask returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasUpdatingTask() bool {
	if o != nil && !IsNil(o.UpdatingTask) {
		return true
	}

	return false
}

// SetUpdatingTask gets a reference to the given string and assigns it to the UpdatingTask field.
func (o *ProvisionUniverseNodesParams) SetUpdatingTask(v string) {
	o.UpdatingTask = &v
}

// GetUpdatingTaskUUID returns the UpdatingTaskUUID field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetUpdatingTaskUUID() string {
	if o == nil || IsNil(o.UpdatingTaskUUID) {
		var ret string
		return ret
	}
	return *o.UpdatingTaskUUID
}

// GetUpdatingTaskUUIDOk returns a tuple with the UpdatingTaskUUID field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetUpdatingTaskUUIDOk() (*string, bool) {
	if o == nil || IsNil(o.UpdatingTaskUUID) {
		return nil, false
	}
	return o.UpdatingTaskUUID, true
}

// HasUpdatingTaskUUID returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasUpdatingTaskUUID() bool {
	if o != nil && !IsNil(o.UpdatingTaskUUID) {
		return true
	}

	return false
}

// SetUpdatingTaskUUID gets a reference to the given string and assigns it to the UpdatingTaskUUID field.
func (o *ProvisionUniverseNodesParams) SetUpdatingTaskUUID(v string) {
	o.UpdatingTaskUUID = &v
}

// GetUpgradeOption returns the UpgradeOption field value
func (o *ProvisionUniverseNodesParams) GetUpgradeOption() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.UpgradeOption
}

// GetUpgradeOptionOk returns a tuple with the UpgradeOption field value
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetUpgradeOptionOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.UpgradeOption, true
}

// SetUpgradeOption sets field value
func (o *ProvisionUniverseNodesParams) SetUpgradeOption(v string) {
	o.UpgradeOption = v
}

// GetUseNewHelmNamingStyle returns the UseNewHelmNamingStyle field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetUseNewHelmNamingStyle() bool {
	if o == nil || IsNil(o.UseNewHelmNamingStyle) {
		var ret bool
		return ret
	}
	return *o.UseNewHelmNamingStyle
}

// GetUseNewHelmNamingStyleOk returns a tuple with the UseNewHelmNamingStyle field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetUseNewHelmNamingStyleOk() (*bool, bool) {
	if o == nil || IsNil(o.UseNewHelmNamingStyle) {
		return nil, false
	}
	return o.UseNewHelmNamingStyle, true
}

// HasUseNewHelmNamingStyle returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasUseNewHelmNamingStyle() bool {
	if o != nil && !IsNil(o.UseNewHelmNamingStyle) {
		return true
	}

	return false
}

// SetUseNewHelmNamingStyle gets a reference to the given bool and assigns it to the UseNewHelmNamingStyle field.
func (o *ProvisionUniverseNodesParams) SetUseNewHelmNamingStyle(v bool) {
	o.UseNewHelmNamingStyle = &v
}

// GetUserAZSelected returns the UserAZSelected field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetUserAZSelected() bool {
	if o == nil || IsNil(o.UserAZSelected) {
		var ret bool
		return ret
	}
	return *o.UserAZSelected
}

// GetUserAZSelectedOk returns a tuple with the UserAZSelected field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetUserAZSelectedOk() (*bool, bool) {
	if o == nil || IsNil(o.UserAZSelected) {
		return nil, false
	}
	return o.UserAZSelected, true
}

// HasUserAZSelected returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasUserAZSelected() bool {
	if o != nil && !IsNil(o.UserAZSelected) {
		return true
	}

	return false
}

// SetUserAZSelected gets a reference to the given bool and assigns it to the UserAZSelected field.
func (o *ProvisionUniverseNodesParams) SetUserAZSelected(v bool) {
	o.UserAZSelected = &v
}

// GetXclusterInfo returns the XclusterInfo field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetXclusterInfo() XClusterInfo {
	if o == nil || IsNil(o.XclusterInfo) {
		var ret XClusterInfo
		return ret
	}
	return *o.XclusterInfo
}

// GetXclusterInfoOk returns a tuple with the XclusterInfo field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetXclusterInfoOk() (*XClusterInfo, bool) {
	if o == nil || IsNil(o.XclusterInfo) {
		return nil, false
	}
	return o.XclusterInfo, true
}

// HasXclusterInfo returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasXclusterInfo() bool {
	if o != nil && !IsNil(o.XclusterInfo) {
		return true
	}

	return false
}

// SetXclusterInfo gets a reference to the given XClusterInfo and assigns it to the XclusterInfo field.
func (o *ProvisionUniverseNodesParams) SetXclusterInfo(v XClusterInfo) {
	o.XclusterInfo = &v
}

// GetYbPrevSoftwareVersion returns the YbPrevSoftwareVersion field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetYbPrevSoftwareVersion() string {
	if o == nil || IsNil(o.YbPrevSoftwareVersion) {
		var ret string
		return ret
	}
	return *o.YbPrevSoftwareVersion
}

// GetYbPrevSoftwareVersionOk returns a tuple with the YbPrevSoftwareVersion field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetYbPrevSoftwareVersionOk() (*string, bool) {
	if o == nil || IsNil(o.YbPrevSoftwareVersion) {
		return nil, false
	}
	return o.YbPrevSoftwareVersion, true
}

// HasYbPrevSoftwareVersion returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasYbPrevSoftwareVersion() bool {
	if o != nil && !IsNil(o.YbPrevSoftwareVersion) {
		return true
	}

	return false
}

// SetYbPrevSoftwareVersion gets a reference to the given string and assigns it to the YbPrevSoftwareVersion field.
func (o *ProvisionUniverseNodesParams) SetYbPrevSoftwareVersion(v string) {
	o.YbPrevSoftwareVersion = &v
}

// GetYbcInstalled returns the YbcInstalled field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetYbcInstalled() bool {
	if o == nil || IsNil(o.YbcInstalled) {
		var ret bool
		return ret
	}
	return *o.YbcInstalled
}

// GetYbcInstalledOk returns a tuple with the YbcInstalled field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetYbcInstalledOk() (*bool, bool) {
	if o == nil || IsNil(o.YbcInstalled) {
		return nil, false
	}
	return o.YbcInstalled, true
}

// HasYbcInstalled returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasYbcInstalled() bool {
	if o != nil && !IsNil(o.YbcInstalled) {
		return true
	}

	return false
}

// SetYbcInstalled gets a reference to the given bool and assigns it to the YbcInstalled field.
func (o *ProvisionUniverseNodesParams) SetYbcInstalled(v bool) {
	o.YbcInstalled = &v
}

// GetYbcSoftwareVersion returns the YbcSoftwareVersion field value if set, zero value otherwise.
func (o *ProvisionUniverseNodesParams) GetYbcSoftwareVersion() string {
	if o == nil || IsNil(o.YbcSoftwareVersion) {
		var ret string
		return ret
	}
	return *o.YbcSoftwareVersion
}

// GetYbcSoftwareVersionOk returns a tuple with the YbcSoftwareVersion field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProvisionUniverseNodesParams) GetYbcSoftwareVersionOk() (*string, bool) {
	if o == nil || IsNil(o.YbcSoftwareVersion) {
		return nil, false
	}
	return o.YbcSoftwareVersion, true
}

// HasYbcSoftwareVersion returns a boolean if a field has been set.
func (o *ProvisionUniverseNodesParams) HasYbcSoftwareVersion() bool {
	if o != nil && !IsNil(o.YbcSoftwareVersion) {
		return true
	}

	return false
}

// SetYbcSoftwareVersion gets a reference to the given string and assigns it to the YbcSoftwareVersion field.
func (o *ProvisionUniverseNodesParams) SetYbcSoftwareVersion(v string) {
	o.YbcSoftwareVersion = &v
}

func (o ProvisionUniverseNodesParams) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ProvisionUniverseNodesParams) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.AdditionalServicesStateData) {
		toSerialize["additionalServicesStateData"] = o.AdditionalServicesStateData
	}
	if !IsNil(o.AllowInsecure) {
		toSerialize["allowInsecure"] = o.AllowInsecure
	}
	if !IsNil(o.Arch) {
		toSerialize["arch"] = o.Arch
	}
	if !IsNil(o.AutoRollbackPerformed) {
		toSerialize["autoRollbackPerformed"] = o.AutoRollbackPerformed
	}
	if !IsNil(o.Capability) {
		toSerialize["capability"] = o.Capability
	}
	if !IsNil(o.ClientRootCA) {
		toSerialize["clientRootCA"] = o.ClientRootCA
	}
	toSerialize["clusters"] = o.Clusters
	if !IsNil(o.CmkArn) {
		toSerialize["cmkArn"] = o.CmkArn
	}
	if !IsNil(o.CommunicationPorts) {
		toSerialize["communicationPorts"] = o.CommunicationPorts
	}
	toSerialize["creatingUser"] = o.CreatingUser
	if !IsNil(o.CurrentClusterType) {
		toSerialize["currentClusterType"] = o.CurrentClusterType
	}
	if !IsNil(o.DeviceInfo) {
		toSerialize["deviceInfo"] = o.DeviceInfo
	}
	if !IsNil(o.EnableYbc) {
		toSerialize["enableYbc"] = o.EnableYbc
	}
	if !IsNil(o.EncryptionAtRestConfig) {
		toSerialize["encryptionAtRestConfig"] = o.EncryptionAtRestConfig
	}
	if !IsNil(o.ErrorString) {
		toSerialize["errorString"] = o.ErrorString
	}
	if !IsNil(o.ExpectedUniverseVersion) {
		toSerialize["expectedUniverseVersion"] = o.ExpectedUniverseVersion
	}
	if !IsNil(o.ExtraDependencies) {
		toSerialize["extraDependencies"] = o.ExtraDependencies
	}
	if !IsNil(o.FipsEnabled) {
		toSerialize["fipsEnabled"] = o.FipsEnabled
	}
	if !IsNil(o.ImportedState) {
		toSerialize["importedState"] = o.ImportedState
	}
	if !IsNil(o.InstallNodeAgent) {
		toSerialize["installNodeAgent"] = o.InstallNodeAgent
	}
	if !IsNil(o.InstallYbc) {
		toSerialize["installYbc"] = o.InstallYbc
	}
	if !IsNil(o.IsKubernetesOperatorControlled) {
		toSerialize["isKubernetesOperatorControlled"] = o.IsKubernetesOperatorControlled
	}
	if !IsNil(o.IsSoftwareRollbackAllowed) {
		toSerialize["isSoftwareRollbackAllowed"] = o.IsSoftwareRollbackAllowed
	}
	if !IsNil(o.ItestS3PackagePath) {
		toSerialize["itestS3PackagePath"] = o.ItestS3PackagePath
	}
	toSerialize["kubernetesUpgradeSupported"] = o.KubernetesUpgradeSupported
	if !IsNil(o.MastersInDefaultRegion) {
		toSerialize["mastersInDefaultRegion"] = o.MastersInDefaultRegion
	}
	if !IsNil(o.NextClusterIndex) {
		toSerialize["nextClusterIndex"] = o.NextClusterIndex
	}
	if !IsNil(o.NodeAgentMissing) {
		toSerialize["nodeAgentMissing"] = o.NodeAgentMissing
	}
	if !IsNil(o.NodeDetailsSet) {
		toSerialize["nodeDetailsSet"] = o.NodeDetailsSet
	}
	if !IsNil(o.NodeExporterUser) {
		toSerialize["nodeExporterUser"] = o.NodeExporterUser
	}
	if !IsNil(o.NodeNames) {
		toSerialize["nodeNames"] = o.NodeNames
	}
	if !IsNil(o.NodePrefix) {
		toSerialize["nodePrefix"] = o.NodePrefix
	}
	if !IsNil(o.NodesResizeAvailable) {
		toSerialize["nodesResizeAvailable"] = o.NodesResizeAvailable
	}
	if !IsNil(o.OtelCollectorEnabled) {
		toSerialize["otelCollectorEnabled"] = o.OtelCollectorEnabled
	}
	if !IsNil(o.PaCollectorUuid) {
		toSerialize["paCollectorUuid"] = o.PaCollectorUuid
	}
	if !IsNil(o.PlacementModificationTaskUuid) {
		toSerialize["placementModificationTaskUuid"] = o.PlacementModificationTaskUuid
	}
	toSerialize["platformUrl"] = o.PlatformUrl
	if !IsNil(o.PlatformVersion) {
		toSerialize["platformVersion"] = o.PlatformVersion
	}
	if !IsNil(o.PrevYBSoftwareConfig) {
		toSerialize["prevYBSoftwareConfig"] = o.PrevYBSoftwareConfig
	}
	if !IsNil(o.PreviousTaskUUID) {
		toSerialize["previousTaskUUID"] = o.PreviousTaskUUID
	}
	if !IsNil(o.RemotePackagePath) {
		toSerialize["remotePackagePath"] = o.RemotePackagePath
	}
	if !IsNil(o.ResetAZConfig) {
		toSerialize["resetAZConfig"] = o.ResetAZConfig
	}
	if !IsNil(o.RollMaxBatchSize) {
		toSerialize["rollMaxBatchSize"] = o.RollMaxBatchSize
	}
	if !IsNil(o.RootAndClientRootCASame) {
		toSerialize["rootAndClientRootCASame"] = o.RootAndClientRootCASame
	}
	if !IsNil(o.RootCA) {
		toSerialize["rootCA"] = o.RootCA
	}
	if !IsNil(o.RunOnlyPrechecks) {
		toSerialize["runOnlyPrechecks"] = o.RunOnlyPrechecks
	}
	if !IsNil(o.SetTxnTableWaitCountFlag) {
		toSerialize["setTxnTableWaitCountFlag"] = o.SetTxnTableWaitCountFlag
	}
	if !IsNil(o.SkipNodeChecks) {
		toSerialize["skipNodeChecks"] = o.SkipNodeChecks
	}
	toSerialize["sleepAfterMasterRestartMillis"] = o.SleepAfterMasterRestartMillis
	toSerialize["sleepAfterTServerRestartMillis"] = o.SleepAfterTServerRestartMillis
	if !IsNil(o.SoftwareUpgradeState) {
		toSerialize["softwareUpgradeState"] = o.SoftwareUpgradeState
	}
	if !IsNil(o.SourceXClusterConfigs) {
		toSerialize["sourceXClusterConfigs"] = o.SourceXClusterConfigs
	}
	if !IsNil(o.SshUserOverride) {
		toSerialize["sshUserOverride"] = o.SshUserOverride
	}
	if !IsNil(o.TargetXClusterConfigs) {
		toSerialize["targetXClusterConfigs"] = o.TargetXClusterConfigs
	}
	if !IsNil(o.UniverseDetached) {
		toSerialize["universeDetached"] = o.UniverseDetached
	}
	if !IsNil(o.UniversePaused) {
		toSerialize["universePaused"] = o.UniversePaused
	}
	if !IsNil(o.UniverseUUID) {
		toSerialize["universeUUID"] = o.UniverseUUID
	}
	if !IsNil(o.UpdateInProgress) {
		toSerialize["updateInProgress"] = o.UpdateInProgress
	}
	if !IsNil(o.UpdateOptions) {
		toSerialize["updateOptions"] = o.UpdateOptions
	}
	if !IsNil(o.UpdateSucceeded) {
		toSerialize["updateSucceeded"] = o.UpdateSucceeded
	}
	if !IsNil(o.UpdatingTask) {
		toSerialize["updatingTask"] = o.UpdatingTask
	}
	if !IsNil(o.UpdatingTaskUUID) {
		toSerialize["updatingTaskUUID"] = o.UpdatingTaskUUID
	}
	toSerialize["upgradeOption"] = o.UpgradeOption
	if !IsNil(o.UseNewHelmNamingStyle) {
		toSerialize["useNewHelmNamingStyle"] = o.UseNewHelmNamingStyle
	}
	if !IsNil(o.UserAZSelected) {
		toSerialize["userAZSelected"] = o.UserAZSelected
	}
	if !IsNil(o.XclusterInfo) {
		toSerialize["xclusterInfo"] = o.XclusterInfo
	}
	if !IsNil(o.YbPrevSoftwareVersion) {
		toSerialize["ybPrevSoftwareVersion"] = o.YbPrevSoftwareVersion
	}
	if !IsNil(o.YbcInstalled) {
		toSerialize["ybcInstalled"] = o.YbcInstalled
	}
	if !IsNil(o.YbcSoftwareVersion) {
		toSerialize["ybcSoftwareVersion"] = o.YbcSoftwareVersion
	}
	return toSerialize, nil
}

type NullableProvisionUniverseNodesParams struct {
	value *ProvisionUniverseNodesParams
	isSet bool
}

func (v NullableProvisionUniverseNodesParams) Get() *ProvisionUniverseNodesParams {
	return v.value
}

func (v *NullableProvisionUniverseNodesParams) Set(val *ProvisionUniverseNodesParams) {
	v.value = val
	v.isSet = true
}

func (v NullableProvisionUniverseNodesParams) IsSet() bool {
	return v.isSet
}

func (v *NullableProvisionUniverseNodesParams) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableProvisionUniverseNodesParams(val *ProvisionUniverseNodesParams) *NullableProvisionUniverseNodesParams {
	return &NullableProvisionUniverseNodesParams{value: val, isSet: true}
}

func (v NullableProvisionUniverseNodesParams) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableProvisionUniverseNodesParams) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
