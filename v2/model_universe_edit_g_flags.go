...
// UniverseEditGFlags struct for UniverseEditGFlags
type UniverseEditGFlags struct {
	// Universe upgrade option to use. * \"Rolling\" - Apply GFlags to each node one at a time * \"Non-rolling\" - Apply GFlags to all nodes at the same time, so has client downtime * \"Non-restart\" - Apply GFlags without restarting nodes or processes. Applicable to only certain supported GFlags.
